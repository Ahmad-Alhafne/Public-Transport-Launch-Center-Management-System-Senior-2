    ...
    return (
        <div className="content-wrapper py-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <h1 style={{margin:'20px 0'}} className="text-3xl font-bold tracking-tight text-[var(--charcoal)]">{t('organizer.profile.title') || 'My Profile'}</h1>
                {!editing ? (
            ...
            <section className="card p-6 mb-6 rounded-[var(--radius)] bg-[var(--surface)] border border-[var(--border)]">
                <h2 className="text-lg font-bold mb-4 pb-1.5 border-b text-[var(--forest-dark)]">{t('organizer.profile.account') || 'Account'}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <label className="flex flex-col gap-1.5">
                        <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">{t('organizer.profile.fullName') || 'Full name'}</span>
                    ...
                    <label className="flex flex-col gap-1.5">
                        <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">{t('organizer.profile.phone') || 'Phone'}</span>
                    ...
                    <label className="flex flex-col gap-1.5">
                        <span className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">{t('organizer.profile.password') || 'Password'}</span>
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={editing ? t('organizer.profile.passwordPlaceholder') || 'New password' : '••••••••'} disabled={!editing} className="input-field" />
                    </label>
                </div>
            </section>
        </div>
